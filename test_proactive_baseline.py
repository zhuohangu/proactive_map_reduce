...
login(token="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")
...
def build_qa_prompt(example):
    q_prompt = '''\n\nTranslate the above texts from English to Chinese. You must only provide the translated text. Do not include any other words or instructions. You must not provide any additional information.''' 
...
def evaluate_dataset(
    ...
    num_layer = 32 # llama-7b: 32, llama-70b: 80, llama-13b: 40
    ...
    for first_doc_k_ratios in first_doc_k_ratios_list:
        for check_layers in check_layers_list:
            ...
            for ex in (tq := tqdm(eval_dataset, desc=f"top_k_ratio: 0, EM:  0.0%")):
                ...
                print(f"DOC CHUNK IDS LENGTH: {doc_chunk_ids_length}")
                ...
                for n in range(num_steps):
                    ...
                    with torch.no_grad():
                        ...
                        for j in range(num_layer):
                            temp_k = past_key_values[j][0][:,:,:input_len].clone()
                    ...
                    # new token that have not been processed yet
                    # input_tensor = torch.tensor([int(torch.argmax(output_dict['logits'][:,-1,:]))], dtype=torch.long).view(1,-1).to(model.device)
                    
                    max_gen_len = round(sum(doc_chunk_ids_length[:n+1])*2.5)
            ...
            candidate = res_check.split()
            bleu_score = sentence_bleu(reference, candidate, smoothing_function=chencherry.method7)
            print(f"reference: {reference}")
            print(f"candidate: {candidate}")
            # bleu = evaluate.load("bleu")
            # print(bleu.compute(predictions=[res_check], references=[gt_text]))
            
            f1 = compute_f1(res_check, gt_text)
            ...
            if output_dir is not None:
                ...
                d = {
                    ...
                    "f1": f1,
                    "outputs": res_check}
                with open(f"{output_dir}baseline_{model_name.split('/')[1]}.json", "a") as f:
