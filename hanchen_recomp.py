...
import torch
import kv_store.kv_loading_interface as kvinterface
    ...
    for first_doc_k_ratios in first_doc_k_ratios_list:
        for check_layers in check_layers_list:
            ...
            for ex in (tq := tqdm(eval_dataset, desc=f"top_k_ratio: 0, EM:  0.0%")):
                ...
                print("length of docprompts is ", len(doc_prompts))
                # print(doc_prompts)

                doc_chunk_ids = [tokenizer.encode(doc)[1:] for doc in doc_prompts]
                # print(len(doc_chunk_ids))
                ...
                for i in range(len(doc_chunk_ids)):
                    ...
                    if i == 0:
                        ...
                        input_chunk_tensors.append(torch.tensor(s_start_1 + temp_input, dtype=torch.long).view(1,-1).to(model.device))                
                # print("doc_chunk_ids length is: ", len(doc_chunk_ids))
                # print("input_chunk_tensors length is :", len(input_chunk_tensors))
                    ...
                    for i in range(len(input_chunk_tensors)):
                        ...
                        for j in range(num_layer):
                            if i == 0:
                                ...
                                kvinterface.add_kv_layer(temp_k, doc_prompts[i], j+1)
                                kvinterface.add_kv_layer(temp_v, doc_prompts[i], -j-1)
                            elif i == 1:
