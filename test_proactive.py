...
def build_qa_prompt(example):
    q_prompt = '''\n\nTranslate the above text from English to Chinese. You must only provide the translated text. Do not include any other words or instructions. You must NOT provide any additional information.\n''' 
        ...
        for check_layers in check_layers_list:
            ...
            for ex in (tq := tqdm(eval_dataset, desc=f"top_k_ratio: 0, EM:  0.0%")):
                ...
                for n in range(num_steps):
                    ...
                    input_ids =  list(chain.from_iterable(temp_doc_chunk_ids)) + q_ids
                    # print(f"INPUTS: {tokenizer.decode(input_ids)}")
                    input_tensor = torch.tensor(input_ids, dtype=torch.long).view(1,-1).to(model.device)
                    input_len = len(input_ids)
                    assert input_len == q_ids_len + sum(doc_chunk_ids_length[:n+1])
                    ...
                    with torch.no_grad():
                        ...
                        for j in range(num_layer):
                            ...
                            if n != 0:
                                output_temp_k = output_past_key_values[j][0][:,:,:].clone()
                                output_temp_v = output_past_key_values[j][1][:,:,:].clone()
                                chunk_past_key_values[j][0] = torch.cat((chunk_past_key_values[j][0],output_temp_k), dim=2)
                                chunk_past_key_values[j][1] = torch.cat((chunk_past_key_values[j][1],output_temp_v), dim=2)
                    ...
                    # If past_key_values is used, input only the last input_ids
                    if n == 0:
                        input_tensor = input_tensor
                    else :
                        input_tensor = torch.tensor(input_ids + total_res_toks, dtype=torch.long).view(1,-1).to(model.device)
                    ...
                    with torch.no_grad():
                        ...
                        print(f"res_toks_len: {len(res_toks)}, source_len: {doc_chunk_ids_length[:n+1]}\n")
                                                
                        # res_check = tokenizer.decode(res_toks)
                        
                        if n < num_steps-1:
                            # Generate kv for output
                            with torch.no_grad():
                                shift = input_len + doc_chunk_ids_length[n+1]+total_res_toks_len
                                
                                print(f"LEN total_res_toks: {len(total_res_toks)}")
                                print(f"LEN shift: {shift}")
                                
                                output_dict = model(torch.tensor(res_toks, dtype=torch.long).view(1,-1).to(model.device), shift=shift)
                                
                                for j in range(num_layer):
                                    if n == 0:
                                        new_output_temp_k = output_dict['past_key_values'][j][0][:,:,:].clone()
                                        new_output_temp_v = output_dict['past_key_values'][j][1][:,:,:].clone()
                                        temp_output_past_key_values.append([new_output_temp_k, new_output_temp_v])
                                    else:
                                        new_output_temp_k = output_dict['past_key_values'][j][0][:,:,:].clone()
                                        new_output_temp_v = output_dict['past_key_values'][j][1][:,:,:].clone()
                                        temp_output_past_key_values[j][0] = torch.cat((output_past_key_values[j][0],new_output_temp_k), dim=2)
                                        temp_output_past_key_values[j][1] = torch.cat((output_past_key_values[j][1],new_output_temp_v), dim=2)
                                output_past_key_values = temp_output_past_key_values
                                output_past_key_values = [tuple(x) for x in output_past_key_values]
                                output_past_key_values = tuple(output_past_key_values)
                                # print(output_past_key_values[0][0].shape)
                ...
                if output_dir is not None:
                    ...
                    with open(f"{output_dir}proactive_{num_steps}_steps_{model_name.split('/')[1]}.json", "a") as f:
...
def main():
    ...
    output_dir = "opus_short_outputs_en_zh/"
